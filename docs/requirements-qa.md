...
**回答**:
- MagicaCloth2 は開発環境でのサンプルモデル（初音ミク）の見栄えのために使用する。FacialControl ライブラリとしては物理演算に依存しない。リリースパッケージには含めない。
...
**回答**:
- 機能単位で段階的にリリースする方式（preview.1 → preview.2 → ... → 1.0.0）。「ミクノオト。」のイベントスケジュールとは独立。
...
- lilToon、MagicaCloth2、初音ミクモデルはリリースパッケージには含めないため、ライセンスの整合性は問題ない。
